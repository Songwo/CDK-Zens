...
export default function NodeListPage() {
  ...
  useEffect(() => {
    if (location.pathname.endsWith("/create")) {
      setForm({ ...EMPTY_NODE, campaignId: campaigns[0]?.id || "" });
    }
  ...
  return (
    <div className="admin-page">
      ...
      <PageHeader
        ...
        actions={<button className="btn btn--primary" onClick={() => setForm({ ...EMPTY_NODE, campaignId: campaigns[0]?.id || "" })}>新建节点</button>}
      />
      ...
      {form && (
        <Modal open={!!form} title="新建分发节点" onCancel={() => setForm(null)}>
