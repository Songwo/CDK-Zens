...
// ── Load all data from MySQL into memory ─────────────────────────

func (s *Store) loadFromMySQL() error {
	db := s.db
	if db == nil {
		return nil
	}
...
// ── Per-entity MySQL save helpers ────────────────────────────────

func (s *Store) dbSaveUser(u *model.User) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO users (id,username,password,role,community_user_id,avatar,nickname,email,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`,
		u.ID, u.Username, u.Password, u.Role, u.CommunityUserID, u.Avatar, u.Nickname, u.Email, u.CreatedAt, u.UpdatedAt)
}

func (s *Store) dbSaveProject(p *model.Project) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO projects (id,name,description,status,creator_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?)`,
		p.ID, p.Name, p.Description, p.Status, p.CreatorID, p.CreatedAt, p.UpdatedAt)
}

func (s *Store) dbDeleteProject(id string) {
	if s.db == nil { return }
	s.db.Exec("DELETE FROM projects WHERE id=?", id)
}

func (s *Store) dbSaveCampaign(c *model.Campaign) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO campaigns (id,project_id,name,description,status,total_stock,claimed_count,remaining_count,start_at,end_at,allow_repeat,per_user_limit,per_ip_limit,per_device_limit,require_captcha_default,project_code,enabled,rules,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
func (s *Store) dbDeleteCampaign(id string) {
	if s.db == nil { return }
	s.db.Exec("DELETE FROM campaigns WHERE id=?", id)
	s.db.Exec("DELETE FROM cdks WHERE campaign_id=?", id)
}

func (s *Store) dbSaveCDK(k *model.CDK) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO cdks (id,campaign_id,code,status,claimed_by_record_id,claimed_at,node_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)`,
		k.ID, k.CampaignID, k.Code, k.Status, k.ClaimedByRecordID, k.ClaimedAt, k.NodeID, k.CreatedAt, k.UpdatedAt)
}

func (s *Store) dbDeleteCDK(id string) {
	if s.db == nil { return }
	s.db.Exec("DELETE FROM cdks WHERE id=?", id)
}

func (s *Store) dbSaveNode(n *model.DistributionNode) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO nodes (id,project_id,campaign_id,name,slug,status,title,description,button_text,require_captcha,show_stock,show_end_time,visits,unique_visitors,claims,failed_claims,last_visited_at,limit_val,ip_limit_enabled,device_limit_enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
func (s *Store) dbSaveClaimRecord(r *model.ClaimRecord) {
	...
	s.db.Exec(`REPLACE INTO claim_records (id,campaign_id,node_id,project_id,cdk_id,code,status,reason,ip,user_agent,fingerprint,idempotency_key,claim_token,hcaptcha_passed,risk_hit,risk_rule_ids,user_id,reward_content,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
...
func (s *Store) dbSaveRiskRule(r *model.RiskRule) {
	...
	s.db.Exec(`REPLACE INTO risk_rules (id,name,type,enabled,config,action,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?)`,
		r.ID, r.Name, r.Type, r.Enabled, string(configJSON), r.Action, r.CreatedAt, r.UpdatedAt)
}

func (s *Store) dbDeleteRiskRule(id string) {
	if s.db == nil { return }
	s.db.Exec("DELETE FROM risk_rules WHERE id=?", id)
}

func (s *Store) dbSaveBlacklist(b *model.BlacklistItem) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO blacklist (id,type,value,reason,enabled,created_at,updated_at) VALUES (?,?,?,?,?,?,?)`,
		b.ID, b.Type, b.Value, b.Reason, b.Enabled, b.CreatedAt, b.UpdatedAt)
}

func (s *Store) dbDeleteBlacklist(id string) {
	if s.db == nil { return }
	s.db.Exec("DELETE FROM blacklist WHERE id=?", id)
}

func (s *Store) dbSaveLog(l *model.SystemLog) {
	if s.db == nil { return }
	metaJSON, _ := json.Marshal(l.Metadata)
	s.db.Exec(`INSERT INTO system_logs (id,type,level,title,message,actor,ip,target_type,target_id,metadata,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?)`,
		l.ID, l.Type, l.Level, l.Title, l.Message, l.Actor, l.IP, l.TargetType, l.TargetID, string(metaJSON), l.CreatedAt)
}

func (s *Store) dbDeleteOldLogs(before string) {
	if s.db == nil { return }
	s.db.Exec("DELETE FROM system_logs WHERE created_at < ?", before)
}

func (s *Store) dbSaveSettings(st model.Settings) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO settings (id,system_name,brand_name,brand_english_name,logo_text,public_base_url,storage_mode,redis_enabled,rabbitmq_enabled,created_at,updated_at) VALUES (1,?,?,?,?,?,?,?,?,?,?)`,
		st.SystemName, st.BrandName, st.BrandEnglishName, st.LogoText, st.PublicBaseURL, st.StorageMode, st.RedisEnabled, st.RabbitMQEnabled, st.CreatedAt, st.UpdatedAt)
}

func (s *Store) dbSaveCaptchaConfig(cc model.CaptchaConfig) {
	if s.db == nil { return }
	s.db.Exec(`REPLACE INTO captcha_config (id,provider,enabled,last_test_status,last_test_message,last_test_at) VALUES (1,?,?,?,?,?)`,
		cc.Provider, cc.Enabled, cc.LastTestStatus, cc.LastTestMessage, cc.LastTestAt)
}

func (s *Store) dbSaveExportTask(t *model.ExportTask) {
	if s.db == nil { return }
	filterJSON, _ := json.Marshal(t.Filter)
	s.db.Exec(`REPLACE INTO export_tasks (id,type,status,filename,file_path,filter,error_msg,created_at,finished_at) VALUES (?,?,?,?,?,?,?,?,?)`,
		t.ID, t.Type, t.Status, t.Filename, t.FilePath, string(filterJSON), t.Error, t.CreatedAt, t.FinishedAt)
}
